...
const GetTab = props => {
  const {item, key, changeSelectedItem, selectedTab} = props
  const {displayText, tabId} = item
  const clsName = selectedTab === tabId ? 'sel-tab-button' : 'tab-button'
  console.log(key)
  const changeState = () => {
    changeSelectedItem(tabId)
  }

  return (
    <div>
      <button className={clsName} onClick={changeState}>
        {displayText}
      </button>
      {selectedTab === tabId ? <hr className="hori-line" /> : null}
    </div>
  )
}

const GetAllTabItems = props => {
  const {item, key, matchImage} = props
...
class App extends Component {
  state = {selectedTab: 'FRUIT', score: 0, timer: 60, end: false, longImg: 0}
  ...
  tick = () => {
    this.setState(prevState => {
      ...
      if (timer === 0) {
        ...
        return {end: true, selectedTab: 'FRUIT'}
      } else {
        if (timer % 5 === 0) {
          return {
            timer: timer - 1,
            longImg: Math.floor(Math.random() * imagesList.length),
          }
        } else {
          return {
            timer: timer - 1,
  ...
  matchImage = id => {
    this.setState(prevState => {
      const {longImg, score} = prevState
      const imageId = imagesList[longImg].id
      if (imageId !== id) {
        clearInterval(this.timeId)
        return {end: true, timer: 0, selectedTab: 'FRUIT'}
      } else {
        return {
          score: score + 1,
          longImg: Math.floor(Math.random() * imagesList.length),
  ...
  startAgain = () => {
    this.setState({score: 0, end: false, timer: 60})
    this.timeId = setInterval(this.tick, 1000)
  }

  render() {
    const {selectedTab, score, timer, longImg, end} = this.state
      ...
      <div className="bg-container">
        <nav>
          <ul className="nav-bar">
            ...
            <li className="nav-con">
              <p className="score-para">
                score: <span className="score-para2">{score}</span>
              </p>
              <img
              ...
              <p className="score-para2">{timer} sec</p>
            </li>
          </ul>
        </nav>

        {end ? (
          <div className="total-game-over-con">
            <div className="game-over">
              <img
                ...
                alt="trophy"
              />
              <p className="game-over-para">Your Score</p>
              <p className="game-over-para">{score}</p>
              <button className="play-button" onClick={this.startAgain}>
                <img
          ...
          <div className="bottom-con">
            <img
              src={imagesList[longImg].imageUrl}
